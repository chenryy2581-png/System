...
let palletSum = {}; // key: driver|location|pallet -> number (累計)
let currentDriver = "";
let currentFish  = "";
let isViewMode   = false;

// ======= 啟動（從 LocalStorage 載入） =======
window.addEventListener('load', () => {
  ...
  // 下拉選單回填
  const dSel = document.getElementById("currentDriver");
  if (currentDriver && ![...dSel.options].some(o=>o.value===currentDriver)) {
    const opt = document.createElement('option'); opt.value = currentDriver; opt.textContent = currentDriver;
    dSel.insertBefore(opt, dSel.querySelector('option[value="_other"]'));
  }
  dSel.value = currentDriver || "";

  const fSel = document.getElementById("currentFish");
  if (currentFish && ![...fSel.options].some(o=>o.value===currentFish)) {
    const opt = document.createElement('option'); opt.value = currentFish; opt.textContent = currentFish;
    fSel.insertBefore(opt, fSel.querySelector('option[value="_other"]'));
  }
  fSel.value = currentFish || "";
...
function renderPalletTable() {
  ...
  Object.keys(palletSum).forEach(key => {
    ...
    let color = "green"; if (total >= 40 && total < 60) color = "orange"; if (total >= 60) color = "red";
    tbody.innerHTML += `
      <tr>
        <td>${driver}</td><td>${location}</td><td>${pallet}</td>
        <td><div class="progress-bar"><div class="progress ${color}" style="width:${percent}%">${total}/60</div></div></td>
        <td>${total}</td><td>${isFull}</td>
      </tr>`;
  });
}
